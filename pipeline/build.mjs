...
// São Paulo: ONE feed from SPTrans carrying the whole city on one sheet —
// 1 345 bus lines (route_type 3), the nine Metrô lines (1) and the seven CPTM
// commuter lines (2), the rail ones with their official colours straight from
// the feed. Ten of the "bus" lines are TROLLEYBUS lines and are drawn green:
// see TROLLEY below for how they were identified.
...
// ONE feed, TWO cfgs: routeTypes splits the SPTrans bundle per mode (3 = bus,
// 1 = Metrô, 2 = CPTM). Without the filter `--all` on the bus mode would
// swallow the rail lines too.
//
// THE TROLLEYBUS LINES. SPTrans ships them as ordinary route_type 3, so the
// feed cannot tell them apart — but two independent sources can, and they
// agree. OpenStreetMap carries nine of them as `route=trolleybus` relations
// (operator Ambiental Transportes Urbanos), and OSM's mapped overhead wire
// (1 506 ways, 167 km — the system is about 137 km) puts every one of those
// nine at 95–100 % of its course under wire. The tenth, 4112-10, has no route
// relation in OSM but runs 100 % under wire, shares the Praça da República
// terminus with 4113-10 and is named as a current trolleybus line by the
// Portuguese Wikipedia, so it joins them. Lines that share the wires without
// being trolleybus-operated stay navy: the night services N401-11, N404-11 and
// N508-11 (the trolleybuses do not run overnight) and the 390E-10 express,
// which OSM maps as `route=bus`.
const TROLLEY = new Set(['2002-10', '2100-10', '2100-21', '2290-10', '2290-21',
  '3160-10', '342M-10', '408A-10', '4112-10', '4113-10']);
// Line keys are SPTrans' own codes, verbatim, for every bus ("8000-10" style).
// The rail lines arrive as "METRÔ L1", "METRÔ 15", "METRÔ17A", "CPTM L07" —
// the operator's name spelled out inside the line name, eight characters where
// two will do on a badge beside a thousand bus codes. They keep the operator's
// initial and the number: M1…M6, M15, M17A, M17W for the Metrô, C7…C13 for the
// CPTM.
...
const lineKey = (sn) => {
  const s0 = (sn || '').trim();
  // Metro runs 1-6/15/17 and CPTM 7-13 — they never collide, and the bus
  // codes are 8000-10 shaped, so the M/C live in the KEY alone. The street
  // (and every SP map) writes "Linha 4", and the official colour says whose.
  let m = /^METR[\u00d4O]\s*L?\s*(\d+[A-Z]?)$/i.exec(s0);
  if (m) { const d = m[1].toUpperCase(); LBL.set('M' + d, d); return 'M' + d; }
  m = /^CPTM\s*L?\s*0*(\d+)$/i.exec(s0);
  if (m) { LBL.set('C' + m[1], m[1]); return 'C' + m[1]; }
  return s0;
};
const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // the city is 70 × 60 km of dense grid — the road graph arrives as a 5 × 5
  // grid of Overpass tiles (a box any bigger just answers 504 here) and is
  ...
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['408A-10']),
  feeds: [
    { tag: 'spt', dir: 'data/gtfs', mapKey: lineKey, routeTypes: ['3'],
      trolley: (r) => TROLLEY.has((r.route_short_name || '').trim()) },
  ],
}];
const tramAll = tramLines.length === 1 && tramLines[0] === 'all';
// The Metrô and the CPTM get the rail-trunk treatment (wide ribbon, station
// discs, always-on names) in the colours the feed ships — the same ones on the
// official diagrams, Linha 1 blue through Linha 13 green.
const isRailTrunk = (l) => /^[MC]\d/.test(l);
if (tramAll || tramLines.length) MODES.push({
  mode: 'tram', label: 'Metrô & CPTM', osmFile: 'data/osm/sp-rail.json',
  graphMode: 'tram', railKeep: new Set(['subway', 'light_rail', 'rail', 'monorail']),
  color: '#d6212b', colorDark: '#7c1116',
  all: tramAll, lines: tramAll ? [] : tramLines,
  feeds: [
    { tag: 'spt', dir: 'data/gtfs', mapKey: lineKey, routeTypes: ['1', '2'] },
  ],
});
  ...
  for (const feed of cfg.feeds) {
    ...
    if (cfg.trolleySet.size) {
      log(`${feed.tag}: ${cfg.trolleySet.size} trolleybus lines (green), ` +
          `${routeToLine.size - cfg.trolleySet.size} motor-bus routes`);
    }
