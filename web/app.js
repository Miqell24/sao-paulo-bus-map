// Frontend: MapLibre GL + OpenFreeMap vector tiles (positron) + SPTrans line layers
...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: GTFS SPTrans (via mobilitydatabase.org)' }));
  ...
  // trolleybus membership is by COLOUR: the pipeline paints the ten of them
  // green, and a name test would need the list kept in two places
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: SPTrans';
      ...
      a.download = `saopaulo-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `saopaulo-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the CPTM runs 60 km out to Jundiaí and Mogi das
  // Cruzes, so fitting the data bbox would shrink the city to a smudge. Open on
  // São Paulo itself, the bus network filling the frame.
  map.jumpTo({ center: [-46.64, -23.58], zoom: 11.0 });
}
